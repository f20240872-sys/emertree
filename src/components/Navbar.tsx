...
export default function Navbar() {
  const [isOpen, setIsOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  const [activeHash, setActiveHash] = useState(""); // Track the hash state
  const pathname = usePathname();

  useEffect(() => {
    // 1. Listen for page scroll changes to toggle background opacity
    const handleScroll = () => {
      if (window.scrollY > 10) {
        setScrolled(true);
      } else {
    ...
    // 2. Sync the hash from the browser on mount and on changes
    ...
    // Set initial hash value if a user lands directly on a hash link
    handleHashChange();
  ...
  // Fixed Active State Matching Logic
  const isActive = (path: string) => {
    // If we're not on the main route, use clean standard matching
    if (pathname !== "/") {
      return pathname.startsWith(path);
    }

    // Extract the hash tag out of our nav link configurations (e.g., "#projects")
    const linkHash = path.split(/(?=#)/)[1] || "";

    if (linkHash === "#home" || linkHash === "") {
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                // Force an update when clicking standard anchors
                onClick={() => setActiveHash(link.href.substring(1))}
                className={`text-sm font-medium transition-all hover:text-emerald-600 ${
                  isActive(link.href)
                    ? "text-emerald-650 font-semibold border-b-2 border-emerald-600 pb-1"
                    : "text-slate-650"
                }`}
